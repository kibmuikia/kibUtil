...
fun String.isMpesaMessage(): Pair<Boolean, Pair<String, String>?> = try {
    /*
    ...
    * */
    ...
    val result = Pair(
        isMpesaMessage, if (isMpesaMessage) {
            Pair(first = code!!, second = phoneNumber!!)
        } else {
            null
        }
    )
    Timber.i(":: isMpesaMessage[ $isMpesaMessage ], result[ $result ]")
    result
} catch (e: Exception) {
